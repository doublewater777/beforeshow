...
pass, starting from the ledger, looks for the redundant **layer**. In the upstream reference campaign,
...
## BeforeShow campaign notes

Define lanes by iOS owner boundaries such as App/root routing, CurrentShow,
Listening, AddShow, Footprints, Memory, Infrastructure, Shared/Widget, and
test/tooling contracts. Baseline and final proof must follow `AGENTS.md`,
use the iPhone 17 simulator, preserve signing entitlements, and treat
`apps/ios/scripts/check_architecture.py` as the canonical architecture owner.
