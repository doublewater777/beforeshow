...
describe("Ticketmaster Discovery API parser", () => {
  ...
  it("routes Ticketmaster IDs to Discovery API instead of page scraping", async () => {
    let requestedUrl = "";
    ...
    const draft = await parseShowLink(EVENT_URL, {
      ticketmasterApiKey: "test-key",
      fetch
    });

    assert.equal(draft.source, "ticketmaster");
    assert.equal(draft.name, "Example Tour");
    assert.equal(new URL(requestedUrl).pathname, `/discovery/v2/events/${EVENT_ID}.json`);
  });
