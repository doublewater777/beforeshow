...
struct TonightFirstListenService {
    static let emptyPrompt = "猜猜那晚会唱什么"
    ...
    func pickSong(
        ...
        let rankedGroups = groups
            .filter { group in
                guard let artistInterestID = group.artistInterestID else {
                    return true
                }
                return !excludedArtistIDs.contains(artistInterestID)
            }
            .sorted { first, second in
                let firstRank = rank(for: first, artistInterests: artistInterests)
                let secondRank = rank(for: second, artistInterests: artistInterests)
    ...
    private func rank(
        for group: CandidateSongGroup,
        artistInterests: [ArtistInterestItem]
    ) -> Int {
        guard let artistInterestID = group.artistInterestID,
              let artist = artistInterests.first(where: { $0.id == artistInterestID }) else {
            return 1
        }

        return artist.status.priority
    }
}
