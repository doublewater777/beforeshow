...
struct ShowDetailView: View {
    ...
    @Environment(\.modelContext) private var modelContext
    @Environment(\.hidesFloatingTabBar) private var hidesFloatingTabBar
    @Query private var selections: [CurrentShowSelection]
    @Query private var notificationStates: [NotificationSchedulingState]
    @Query(sort: \Show.date) private var shows: [Show]
    let show: Show
    ...
    /// 首页 / 我的现场点进来直接展示表单信息：只读进入，点按内容或「编辑」进入可编辑状态。
    var body: some View {
        ShowDraftEditorView(
            title: "现场信息",
        ...
        .onAppear { hidesFloatingTabBar.wrappedValue = true }
        .onDisappear { hidesFloatingTabBar.wrappedValue = false }
    }

    /// 现场状态管理并入编辑器（状态卡 + 立即生效的操作），这里注入编辑器所需的上下文。
